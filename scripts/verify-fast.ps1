...
Write-Host '== [1/4] dotnet build ==' -ForegroundColor Cyan
dotnet build (Join-Path $root 'LinuxWebTool.slnx') -c Release --nologo -v q
if ($LASTEXITCODE -ne 0) { throw '编译失败' }

Write-Host '== [2/4] dotnet test (架构门禁) ==' -ForegroundColor Cyan
...
Write-Host '== [3/4] dotnet test (API 集成门禁) ==' -ForegroundColor Cyan
dotnet test (Join-Path $root 'tests/LinuxWebTool.IntegrationTests/LinuxWebTool.IntegrationTests.csproj') -c Release --no-build --nologo
if ($LASTEXITCODE -ne 0) { throw 'API 集成测试失败' }

if (-not $SkipFrontend) {
    Write-Host '== [4/4] 前端架构门禁 ==' -ForegroundColor Cyan
